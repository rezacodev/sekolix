...
export async function seedRegistrationSettings(years: Array<{ id: string; label: string; isActive: boolean; yearCode: string | null; }>) {
  ...
  for (const setting of settings) {
    ...
    if (!regSetting) {
      ...
      console.log(`✓ Created registration setting for year: ${activeYear.label}`);
    } else {
      console.log(`ℹ️  Registration setting already exists for year: ${activeYear.label}`);
    }
...
export async function seedApplicants(registrationSettings: Array<{ id: string; prefix: string; currentNumber: number; }>, years: Array<{ id: string; label: string; isActive: boolean; yearCode: string | null; }>, numApplicants = 50) {
  console.log(`👥 Seeding ${numApplicants} applicants...`);

  const applicants: Array<{ id: string; registrationCode: string | null; fullName: string; }> = [];
  ...
  for (let i = 0; i < numApplicants; i++) {
    ...
    const applicantData = {
      ...
      // Status
      status: getRandomItem([ApplicantStatus.pending, ApplicantStatus.review, ApplicantStatus.accepted, ApplicantStatus.rejected]),
