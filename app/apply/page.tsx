          ...
          <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6 lg:px-8">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="max-w-3xl">
                <p className="text-sm font-semibold uppercase tracking-[0.3em] text-muted-foreground">Penerimaan Siswa</p>
                ...
                {activeYear && (
                  <p className="mt-3 text-sm text-muted-foreground">Tahun ajaran aktif: {activeYear.label}</p>
                )}
              </div>
              <div className="flex flex-wrap justify-center gap-3 text-sm font-semibold">
                <Link href="/" className="rounded-full border border-card px-4 py-2 text-foreground transition hover:border-card hover:bg-muted">
                  Kembali ke Homepage
                </Link>
                <Link href="#apply-form" className="rounded-full border border-card bg-muted px-4 py-2 text-foreground transition hover:bg-muted/80">
                  Daftar
                </Link>
                <Link href="/apply/login" className="rounded-full border border-card bg-muted px-4 py-2 text-foreground transition hover:bg-muted/80">
                  Login
                </Link>
              </div>
            </div>
        ...
        <div className="mx-auto max-w-4xl space-y-8 px-4 py-16 sm:px-6 lg:px-8">
          {!isApplyFormEnabled && (
            <div className="rounded-lg border border-card bg-muted px-4 py-3 text-sm text-foreground">
              ...
              <p className="mt-1">Maaf, form pendaftaran saat ini tidak tersedia. Silakan hubungi sekolah untuk informasi lebih lanjut.</p>
            </div>
          )}

          {isApplyFormEnabled && (
            <div id="apply-form" className="rounded-3xl border border-card bg-card p-8 shadow-2xl shadow-slate-900/50 backdrop-blur">
              <div className="space-y-4">
                <h2 className="text-2xl font-semibold text-foreground">Formulir Pendaftaran</h2>
                <p className="text-sm text-muted-foreground">
                  Pastikan NIK dan nomor HP sudah sesuai dokumen. Data kamu akan dilindungi sesuai kebijakan privasi sekolah.
                </p>
              </div>
              <div className="mt-8">
