...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!applicant) {
      return NextResponse.json({ message: "Peserta didik tidak ditemukan" }, { status: 404 });
    }
    ...
    if (profileData.fatherBirthYear)
      updateData.fatherBirthYear = parseInt(String(profileData.fatherBirthYear));
    if (profileData.fatherEducation)
      updateData.fatherEducation = String(profileData.fatherEducation);
    if (profileData.fatherOccupation)
      updateData.fatherOccupation = String(profileData.fatherOccupation);
    ...
    if (profileData.motherBirthYear)
      updateData.motherBirthYear = parseInt(String(profileData.motherBirthYear));
    if (profileData.motherEducation)
      updateData.motherEducation = String(profileData.motherEducation);
    if (profileData.motherOccupation)
      updateData.motherOccupation = String(profileData.motherOccupation);
    ...
    if (profileData.guardianBirthYear)
      updateData.guardianBirthYear = parseInt(String(profileData.guardianBirthYear));
    if (profileData.guardianEducation)
      updateData.guardianEducation = String(profileData.guardianEducation);
    if (profileData.guardianOccupation)
      updateData.guardianOccupation = String(profileData.guardianOccupation);
    ...
    if (profileData.distanceToSchool)
      updateData.distanceToSchool = parseFloat(String(profileData.distanceToSchool));
    if (profileData.transportationMode)
      updateData.transportationMode = String(profileData.transportationMode);
    if (profileData.anak_ke) updateData.anakKe = parseInt(String(profileData.anak_ke));
    if (profileData.jumlahSaudara)
      updateData.jumlahSaudara = parseInt(String(profileData.jumlahSaudara));
    ...
    return NextResponse.json(
      {
        message: isComplete
          ? "Data profil berhasil disimpan dan selesai."
          : "Progress pengisian data berhasil disimpan.",
        applicant: {
          id: updated.id,
