...
export default function MediaActions() {
  ...
  const confirmDeleteMedia = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/website-landing/media/${confirmDelete}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="space-y-4 p-4 md:p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Perpustakaan Media</h2>
          <p className="text-muted-foreground">Kelola gambar dari Cloudinary.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
          {mediaItems.map((item) => {
            ...
            return (
              <div key={item.id} className="group rounded-lg border bg-card text-card-foreground shadow-sm overflow-hidden">
                ...
                <div className="p-3 flex items-center justify-between">
                  <p className="text-sm font-medium line-clamp-1" title={item.title || undefined}>{item.title || "Tanpa judul"}</p>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-md overflow-hidden border bg-muted h-64 flex items-center justify-center">
                {previewItem.type === "image" ? (
                  <div className="relative w-full h-full">
                    <Image src={previewItem.url} alt={previewItem.title || "Media"} fill className="object-contain" />
                  </div>
                ) : (
                  <div className="p-6 text-center text-sm text-muted-foreground">Pratinjau tidak tersedia untuk tipe ini.</div>
                )}
              </div>
              <div className="space-y-2 text-sm">
                <div className="grid grid-cols-[100px_auto_1fr] gap-x-3 gap-y-2">
                  ...
                  <span className="text-muted-foreground">:</span>
                  <a href={previewItem.url} target="_blank" rel="noreferrer" className="text-primary hover:underline break-all">
                    {previewItem.url}
                  </a>
