...
export default function FasilitasEditor({ pageId }: { pageId: string }) {
  ...
  const [selectedItemForImage, setSelectedItemForImage] = useState<{ catIndex: number; itemIndex: number } | null>(null);
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.data?.categories) {
            ...
            setCategories([
              { title: "Workshop & Laboratorium", items: [] },
            ]);
          }
        }
      } catch (error) {
  ...
  const handleSelectImageFromMedia = (media: unknown) => {
    ...
    const newCategories = [...categories];
    newCategories[selectedItemForImage.catIndex].items[selectedItemForImage.itemIndex].image = (media as { url: string }).url;
    setCategories(newCategories);
    ...
    <>
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Kelola fasilitas dan infrastruktur sekolah</p>
        </div>
        ...
        <Card key={catIndex}>
          ...
          <CardContent className="space-y-4">
            {category.items.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-8">Belum ada item dalam kategori ini.</p>
            ) : (
              category.items.map((item, itemIndex) => (
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor={`item-name-${catIndex}-${itemIndex}`} className="text-sm mb-2 block">
                        Nama Fasilitas
                      </Label>
                      <Input
                    ...
                    <div>
                      <Label htmlFor={`item-image-${catIndex}-${itemIndex}`} className="text-sm mb-2 block">
                        Gambar Fasilitas
                      </Label>
                  ...
                  <div>
                    <Label htmlFor={`item-desc-${catIndex}-${itemIndex}`} className="text-sm mb-2 block">
                      Deskripsi
                    </Label>
                    <textarea
