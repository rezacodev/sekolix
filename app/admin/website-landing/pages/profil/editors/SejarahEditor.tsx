...
export default function SejarahEditor({ pageId }: { pageId: string }) {
  ...
  const handleRemoveTimelineItem = (index: number) => {
    showConfirm(
      "Hapus Timeline?",
      "Timeline ini akan dihapus dan tidak dapat dipulihkan.",
      () => {
    ...
    <>
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Kelola konten dan timeline halaman Sejarah</p>
        </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="content" className="mb-2 block">Deskripsi Halaman</Label>
            <RichTextEditor
              content={content}
              onChange={setContent}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {timeline.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-8">Belum ada timeline. Tambahkan timeline pertama Anda.</p>
          ) : (
