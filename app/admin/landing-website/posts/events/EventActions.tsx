...
export default function EventActions() {
  ...
  const fetchEvents = async (p = 0, ps = 10, s = "") => {
    ...
    try {
      ...
      if (s) q.set("search", s);
      const res = await fetch(`/api/admin/landing-website/events?${q.toString()}`);
      if (!res.ok) {
  ...
  const confirmDeleteEvent = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/landing-website/events/${confirmDelete}`, {
        method: "DELETE"
      });
  ...
  const handleEdit = (id: string) => {
    router.push(`/admin/landing-website/posts/events/${id}/edit`);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <AdminPageHeader title="Acara" description="Kelola acara dan kalender sekolah" />
        </div>
        <Link href="/admin/landing-website/posts/events/new">
          <Button>
