...
export default function GalleryActions() {
  ...
  const fetchAlbums = async () => {
    try {
      const response = await fetch("/api/admin/landing-website/gallery/albums");
      if (response.ok) {
        const data = await response.json();
        setAlbums(data);
      }
  ...
  const handleCreateAlbum = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingAlbumId
        ? `/api/admin/landing-website/gallery/albums/${editingAlbumId}`
        : "/api/admin/landing-website/gallery/albums";
  ...
  const confirmDeleteAlbum = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/landing-website/gallery/albums/${confirmDelete.id}`,
        {
          method: "DELETE"
        }
      );
  ...
  const fetchGalleries = async () => {
    try {
      const response = await fetch("/api/admin/landing-website/gallery");
      if (response.ok) {
        const data = await response.json();
        setGalleries(data);
        setReorderList(data);
  ...
  const onSubmit = async (values: GalleryFormValues) => {
    ...
    try {
      const url = editingId
        ? `/api/admin/landing-website/gallery/${editingId}`
        : "/api/admin/landing-website/gallery";
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch("/api/admin/landing-website/media", {
        method: "POST",
        body: formData
      });
  ...
  const confirmDeleteGallery = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/landing-website/gallery/${confirmDelete.id}`, {
        method: "DELETE"
      });
  ...
  const columns = createColumns(handleEdit, handleDelete, busyId, {
    draggable: true,
  ...
  const saveOrder = async (list: Gallery[]) => {
    try {
      const response = await fetch("/api/admin/landing-website/gallery/reorder", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <AdminPageHeader title="Galeri" description="Kelola galeri foto dan gambar" />
        </div>
      ...
      >
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingId ? "Sunting Item Galeri" : "Tambah Gambar Baru"}</DialogTitle>
            <DialogDescription>
              {editingId
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormItem>
                      ...
                      <div className="space-y-3">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Jika Anda mengunggah dari PC atau memilih dari Perpustakaan, URL akan
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Organisir foto berdasarkan album</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Angka lebih kecil tampil terlebih dahulu</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                <DialogFooter>
                  ...
                  <Button
                    type="submit"
                    disabled={isLoading || isUploading || !form.getValues("image")}
                  >
                    {isLoading ? "Menyimpan..." : editingId ? "Perbarui Gambar" : "Tambah Gambar"}
                  </Button>
                </DialogFooter>
              </form>
            </Form>
              ...
              <div className="space-y-2">
                ...
                {albums.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    Belum ada album. Buat satu di atas.
                  </p>
                ) : (
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {albums.map(album => (
                      <div
                        key={album.id}
                        className="flex items-center justify-between bg-card border border-card rounded p-3"
                      >
      ...
      <Dialog open={showMediaModal} onOpenChange={setShowMediaModal}>
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Pilih gambar dari perpustakaan media Anda</DialogDescription>
          </DialogHeader>
