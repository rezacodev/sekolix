...
export default function AlbumsPage() {
  ...
  const onSubmit = async (values: AlbumFormValues) => {
    ...
    try {
      const url = editingId
        ? `/api/admin/landing-website/gallery/albums/${editingId}`
  ...
  const confirmDeleteAlbum = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/landing-website/gallery/albums/${confirmDelete}`, {
        method: "DELETE"
      });
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <AdminPageHeader title="Gallery Albums" description="Manage photo album categories" />
        </div>
            ...
            {isFetching ? (
              ...
              albums.map(album => (
                <TableRow key={album.id}>
                  ...
                  <TableCell className="text-center">{album._count?.galleries || 0}</TableCell>
                  <TableCell>
                    <div className="flex gap-2">
                      <Button variant="ghost" size="sm" onClick={() => handleEdit(album)}>
                        <Edit2 className="h-4 w-4" />
                      </Button>
                      <Button variant="ghost" size="sm" onClick={() => handleDelete(album.id)}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>Brief description to help identify the album</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
