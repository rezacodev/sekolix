...
export default function PageActions() {
  ...
  const fetchPages = async (p = 0, ps = 10, s = "") => {
    ...
    try {
      ...
      if (s) q.set("search", s);
      const res = await fetch(`/api/admin/landing-website/pages?${q.toString()}`);
      if (!res.ok) {
        setPages([]);
        setTotalCount(0);
        return;
  ...
  const confirmDeletePage = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/landing-website/pages/${confirmDelete}`, {
        method: "DELETE"
      });
  ...
  const handleToggleVisibility = async (id: string, isVisible: boolean) => {
    try {
      const response = await fetch(`/api/admin/landing-website/pages/${id}`, {
        method: "PATCH",
        headers: {
  ...
  const handleEdit = (id: string) => {
    router.push(`/admin/landing-website/pages/${id}/edit`);
  };
    ...
    <div className="space-y-4">
      <div>
        <AdminPageHeader
          ...
          description="Kelola 5 halaman profil: Sejarah, Visi & Misi, Struktur, Fasilitas, dan Program Keahlian."
        />
      </div>
      {isLoading ? (
