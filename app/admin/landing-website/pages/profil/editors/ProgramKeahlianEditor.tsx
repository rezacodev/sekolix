...
export default function ProgramKeahlianEditor({ pageId }: { pageId: string }) {
  ...
  const handleAddProgram = () => {
    setPrograms([...programs, { name: "", code: "", description: "", duration: "3 Tahun" }]);
  };

  const handleRemoveProgram = (index: number) => {
    showConfirm("Hapus Program?", "Program ini akan dihapus dan tidak dapat dipulihkan.", () => {
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Kelola program-program keahlian yang tersedia
            </p>
          </div>
          ...
          <CardContent className="space-y-4">
            {programs.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-8">
                Belum ada program keahlian. Tambahkan program pertama Anda.
              </p>
            ) : (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input
                        id={`prog-code-${index}`}
                        value={program.code}
                        onChange={e => handleProgramChange(index, "code", e.target.value)}
