...
export default function PagesList() {
  ...
  const fetchPages = async () => {
    try {
      const response = await fetch("/api/admin/landing-website/pages");
      if (response.ok) {
  ...
  const handleToggleVisibility = async (e: React.MouseEvent, slug: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/landing-website/pages/${page.id}`, {
        method: "PATCH",
          ...
          {profilePages.map(profilePage => {
            ...
            return (
              <Link
                ...
                href={`/admin/landing-website/pages/profil/${profilePage.slug}`}
                className="group"
              >
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between pt-2 border-t">
                        ...
                        >
                          ...
                          <span className="text-xs">{isVisible ? "Sembunyikan" : "Tampilkan"}</span>
                        </button>
