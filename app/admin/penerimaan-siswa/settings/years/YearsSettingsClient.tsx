...
export function YearsSettingsClient({
  initialYears,
  ...
  const mapRow = useCallback(
    (y: TahunAjaranRow): Year => ({
      ...
      isActive: !!y.isActive,
      registrationFee:
        typeof y.registrationFee === "number" ? y.registrationFee : Number(y.registrationFee) || 0
    }),
    []
  );

  const refreshYears = useCallback(
    async (p = pageIndex, ps = pageSize, s = search) => {
      setIsLoading(true);
    ...
    const cols: ColumnDef<Year>[] = [
      { accessorKey: "name", header: "Nama" },
      {
        ...
        cell: ({ row }: { row: { original: Year } }) => (
          <span>{row.original.start ? formatDate(row.original.start) : "—"}</span>
        )
      },
      {
        accessorKey: "end",
        header: "Selesai",
        cell: ({ row }: { row: { original: Year } }) => (
          <span>{row.original.end ? formatDate(row.original.end) : "—"}</span>
        )
      }
    ];
        ...
        cell: ({ row }: { row: { original: Year } }) => (
          <span>
            {row.original.registrationFee === 0
              ? "Gratis"
              : formatRupiah(row.original.registrationFee || 0)}
          </span>
        )
      });
    }
          ...
          <DropdownMenuContent>
            ...
            {!row.original.isActive ? (
              <DropdownMenuItem
                ...
                onSelect={() => handleToggleYear(row.original.id, row.original.isActive)}
              >
                Aktifkan
              </DropdownMenuItem>
            ) : (
              <DropdownMenuItem disabled>Saat ini aktif</DropdownMenuItem>
            )}
          </DropdownMenuContent>
        </DropdownMenu>
      )
    ...
    <div className="space-y-4">
        ...
        {!hideTitle && (
          <div>
            ...
            <p className="text-muted-foreground">
              Kelola tahun ajaran yang tersedia untuk pendaftaran.
            </p>
          </div>
        )}
      ...
      <div>
        <DataTable
          ...
          onPageChange={p => {
            setPageIndex(p);
            void refreshYears(p, pageSize, search);
          }}
          onPageSizeChange={ps => {
            setPageSize(ps);
            void refreshYears(0, ps, search);
          }}
          onSearchChange={s => {
            setSearch(s);
            setPageIndex(0);
            void refreshYears(0, pageSize, s);
          }}
        />
      </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {editing ? "Perbarui detail tahun ajaran." : "Tambahkan tahun ajaran baru."}
            </DialogDescription>
          </DialogHeader>
