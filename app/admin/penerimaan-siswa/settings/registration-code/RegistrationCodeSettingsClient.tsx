...
export function RegistrationCodeSettingsClient({
  initialYears,
}: {
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        "/api/admin/penerimaan-siswa/settings/registration-code",
        {
          method: "POST",
  ...
  return (
    <div className="rounded-3xl border border-card bg-card p-8 shadow space-y-6">
        ...
        <section>
          ...
          <p className="text-muted-foreground">Kelola format dan nomor urut kode registrasi untuk setiap tahun ajaran secara independen</p>
        </section>
          ...
          <div className="space-y-6">
                ...
                <div className="rounded-lg border border-card bg-card p-4">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="inline-flex items-center gap-3">
                      <code className="text-2xl font-mono font-bold text-foreground bg-muted px-3 py-1 rounded-lg">{previewCode}</code>
                    </div>
                    <div className="text-sm text-muted-foreground">Contoh kode berikutnya akan digunakan saat ada pendaftar baru</div>
                  </div>
                </div>
                  ...
                  <div className="grid gap-6 lg:grid-cols-2">
                    ...
                    <label className="block">
                      ...
                      <p className="mt-2 text-xs text-muted-foreground">
                        Kode tahun: {selectedYear.yearCode || "belum diatur"} - akan ditambahkan setelah prefix
                      </p>
                    </label>
                  </div>
            ...
            {!selectedYearId && (
              <div className="rounded-lg border border-card bg-muted p-4 text-sm text-muted-foreground">
                ...
                <p className="mt-1 text-xs">Setiap tahun ajaran memiliki pengaturan kode registrasi yang terpisah dan independen</p>
              </div>
            )}
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-card bg-card p-6 shadow">
          ...
          <ul className="space-y-3 text-sm text-foreground">
            <li className="flex items-start gap-3">
              ...
              <span>Setiap tahun ajaran memiliki pengaturan kode registrasi yang <strong>independen</strong></span>
            </li>
            <li className="flex items-start gap-3">
              <span className="text-accent font-semibold mt-0.5">•</span>
            ...
            <li className="flex items-start gap-3">
              <span className="text-accent font-semibold mt-0.5">•</span>
              <span>Pastikan kode tahun ajaran sudah diatur di menu Tahun Ajaran terlebih dahulu</span>
            </li>
          </ul>
        </section>
    </div>
